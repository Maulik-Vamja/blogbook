                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        <h3 class="card-title">All Comments &nbsp;
                            <span class="badge"> {{ $comments->count() }} </span></h3>
                        <div class="toolbar">
                        </div>
                            ...
                            <table id="datatables" class="table table-striped table-no-bordered table-hover"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <p>
                                                <h4><strong> {{ $comment->user->username }} </strong><small>
                                                        {{ $comment->created_at->diffForHumans() }} </small></h4>
                                                <p>{{ $comment->comment }}
                                                </p>
                                                <a target="_blank"
                                                    href=" {{  route('post.details',$comment->post->slug.'#btnComment') }}">
                                                    <p>Replay</p>
                                                </a>
                                            </p>
                                        </td>
                                        <td>
                                            <a href="javascript::void(0)">
                                                <img class="img-thumbnail  img-responsive"
                                                    src=" {{ Storage::disk('public')->url('posts/'.$comment->post->image) }} "
                                        ...
                                        <td>
                                            <span>
                                                <a href=" {{ route('post.details',$comment->post->slug) }} ">
                                                    <h4><strong> {{ str_limit($comment->post->title,22) }} </strong>
                                                    </h4>
                                                </a>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <form id="remove-from-{{ $comment->id }}" method="POST"
